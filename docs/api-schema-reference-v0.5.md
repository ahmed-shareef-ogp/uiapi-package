...
<p>
    <img alt="Manual Version v0.5" src="https://img.shields.io/badge/Manual-v0.5-F59E0B?style=for-the-badge">
</p>

</div>

> This document covers every apiSchema key, its purpose, accepted values, defaults, and what happens when it is missing. v0.4 added a complete **Error Reference** covering every error response the GAPI and CCS endpoints can return, plus a guide to common apiSchema mistakes and their consequences. v0.5 adds UUID-based record lookup and enforcement.
...
<table>
    ...
    <tr>
        <td><a href="#error-reference"><strong>12.</strong> Error Reference</a></td>
    </tr>
</table>
...
> **Tip:** Run the `ViewConfigValidator` against your view config — it will catch many of these issues automatically. See the [Error Reference](#error-reference) for the exact error responses each mistake produces at runtime.

</details>

## Error Reference

This section documents every error response the GAPI and CCS endpoints can return, the JSON shape of each response, what triggers it, and how to fix it.

---

### UUID-Based Record Lookup

The `{id}` segment in show (`GET`), update (`PUT`), and delete (`DELETE`) endpoints accepts either an **integer primary key** or a **UUID**. When a UUID is passed, the controller looks up the record by UUID column instead of the integer primary key.

#### Resolution order

1. If `{id}` matches UUID format (`xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`):
   - Use the model's `$uuidColumn` property if declared (e.g., `protected string $uuidColumn = 'external_id'`)
   - Otherwise fall back to a column literally named `uuid` if it exists on the table
   - If neither exists, fall back to integer ID lookup
2. If `{id}` is not UUID-shaped → integer ID lookup directly

#### UUID enforcement

When `enforce_uuid` is set to `true` in `config/uiapi.php`, integer ID lookups are **rejected with a `404`** for any model that has a UUID column. Only UUID-based lookups are accepted for those models. Models without a UUID column are unaffected.

This is a server-side configuration — frontend clients should always use UUIDs for models that have a `uuid` column.

#### Declaring a custom UUID column

By default the controller looks for a column named `uuid`. If your model uses a differently-named UUID column, declare it on the model:
...
No other changes are needed — the controller picks this up automatically.

---

### GAPI Endpoint Errors

All GAPI error responses are JSON objects. The `message` key is always present. A localized variant is returned based on the `?lang=` query parameter (`dv` by default, `en` if `?lang=en`). When `APP_DEBUG=true`, the raw PHP exception message is appended to `message`.

#### Response shapes

**Error without field details:**
```json
{
...
**Validation / constraint error with field details:**
```json
{
...
| HTTP Status | `message` (en) | Cause | Fix |
|-------------|----------------|-------|-----|
| `400` | `"Model not found."` | No class found matching `{model}` in `App\Models\` or `Ogp\UiApi\Models\` | Check spelling; model class must exist and be autoloaded |
...
| HTTP Status | `message` (en) | Cause | Fix |
|-------------|----------------|-------|-----|
| `400` | `"Model not found."` | Model class not found | Create the model or fix the URL segment |
| `404` | `"Record not found."` | No row with that `{id}` exists | Verify the `{id}` is correct |
| `404` | `"Record not found."` | Integer ID passed to a UUID-enabled model with `enforce_uuid: true` | Use the record's UUID instead of its integer ID |
| `500` | `"Something went wrong while processing your request."` | Unhandled exception during query or payload processing | Check Laravel logs; enable `APP_DEBUG=true` for the raw message |
...
| HTTP Status | `message` (en) | `errors` present? | Cause | Fix |
|-------------|----------------|:-----------------:|-------|-----|
| `400` | `"Invalid resource type."` | No | Model class not found | Ensure the model class exists and is named correctly |
| `422` | `"Validation failed."` | Yes — field map | Laravel validation rules in `baseRules()` / `rulesForCreate()` rejected the input | Review the `errors` map; fix the submitted payload |
| `422` | `"Unable to save the record."` | Yes — DB error | Database constraint violation (unique, foreign key, NOT NULL) | See [Database Constraint Errors](#database-constraint-errors) below |
| `500` | `"Something went wrong while processing your request."` | No | Unhandled exception | Check Laravel logs |

</details>

---

#### PUT /api/gapi/{model}/{id}
...
| HTTP Status | `message` (en) | `errors` present? | Cause | Fix |
|-------------|----------------|:-----------------:|-------|-----|
| `400` | `"Model not found."` | No | Model class not found | Create or fix the model class |
| `404` | `"Record not found."` | No | Integer ID passed to a UUID-enabled model with `enforce_uuid: true` | Use the record's UUID instead of its integer ID |
| `422` | `"Validation failed."` | Yes — field map | `rulesForUpdate()` / `baseRules()` rejected the input | Review the `errors` map |
| `422` | `"Unable to update the record."` | Yes — DB error | Database constraint violation | See [Database Constraint Errors](#database-constraint-errors) below |
| `500` | `"Something went wrong while processing your request."` | No | Unhandled exception | Check Laravel logs |
...
| HTTP Status | `message` (en) | Cause | Fix |
|-------------|----------------|-------|-----|
| `400` | `"Model not found."` | Model class not found | Create or fix the model class |
| `404` | `"Record not found."` | No row with that `{id}` exists | Verify the `{id}` is correct |
| `404` | `"Record not found."` | Integer ID passed to a UUID-enabled model with `enforce_uuid: true` | Use the record's UUID instead of its integer ID |
...
When a `QueryException` is caught (on store or update), the controller parses it into a structured `errors` map. The `message` tells you the operation that failed; `errors` tells you which column and why.
...
```json
{
    ...
    "errors": {
        "column_name": ["The value for column_name already exists."]
    }
}
```

<details open>
<summary><strong>Constraint error types</strong></summary>

| DB Error | MySQL code | PostgreSQL code | `errors` value |
|----------|:----------:|:---------------:|----------------|
| **Unique violation** | `1062` | `23505` | `{ "column": ["The value for column already exists."] }` |
| **Foreign key violation** | `1452` | `23503` | `{ "column": ["The referenced record for column does not exist."] }` |
| **NOT NULL violation** | `1048` | `23502` | `{ "column": ["The column field cannot be empty."] }` |
| **Other DB error** | — | — | `{ "database": ["A database error occurred."] }` |

</details>

**When `errors` falls back to `"database"`:** The exception message did not contain a parseable column name (e.g., a composite index or an unfamiliar SQL dialect). In this case `errors.database` contains a generic message. Enable `APP_DEBUG=true` to see the raw SQL error appended to `message`.

---

### CCS Endpoint Errors

All CCS errors return HTTP `422` with a JSON body containing an `"error"` key (not `"message"`):

```json
{
    "error": "Model 'Foo' not found or missing apiSchema()"
}
```

When `debug_level >= 2` is set in `config/uiapi.php` and the view config fails validation, the response also contains a `"validation"` sub-key:
...
```json
{
    ...
    "validation": {
        "errors": [
            { "path": "table.columns[0]", "rule": "column_in_schema", "message": "..." }
        ],
        "warnings": []
    }
}
...
<details open>
<summary><strong>CCS error messages and their causes</strong></summary>

| `error` value | Cause | Fix |
|---------------|-------|-----|
| `"View config file missing for model '{model}'"` | No JSON file found in the configured `viewConfigs` path | Create `app/Services/viewConfigs/{model}.json` (lowercase) |
| `"Model '{model}' not found or missing apiSchema()"` | Model class does not exist, or exists but does not implement `apiSchema()` | Create the model extending `BaseModel` and add `apiSchema()` |
| `"View '{viewKey}' not found in config"` | The `?view=` value does not match any key in the view config JSON, and no fallback view exists | Check the view key spelling; all view keys must contain `"View"` in the name |
| `"Invalid view key '{viewKey}'. View keys must contain 'View' in the name."` | The `?view=` parameter was provided but does not include the word `View` | Rename to e.g. `listView`, `editView`, or any key containing `View` |
| `"View '{viewKey}' does not have a valid 'components' configuration"` | The view block exists but its `components` key is missing or not an array | Add a `components` array to the view block in the JSON |
| `"noModel mode requires columnsSchema in view config"` | A component has `"noModel": true` but no `columnsSchema` block is defined | Add `columnsSchema` to the component or view config block |
| `"component parameter is required"` | The CCS was called without a `?component=` or `?view=` parameter | Add `?component=table` or `?view=listView` to the request |
| `"View config validation failed"` | View config has structural errors (only checked when `debug_level >= 2`) | Read the `validation.errors` array for specific rule failures |
| `"Invalid columns segment '{token}'"` | A `columns` parameter in the view config contains a malformed token | Check for typos in the `columns` array in the view config JSON |
| `"Unknown relation reference '{relation}' in columns"` | A `relation.column` token references a relationship method that does not exist on the model | Add the relationship method to the model or fix the column name |
| `"Related model for '{relation}' lacks apiSchema()"` | The related model was resolved but has no `apiSchema()` method | Implement `apiSchema()` on the related model |
| `"Column '{column}' is not defined in {model} apiSchema"` | A column listed in the view config's `columns` array is not present in `apiSchema()` | Add the column to `apiSchema()` or remove it from the view config |
...
When a `POST` or `PUT` returns `422` with `"message": "Validation failed."`, the `errors` object maps each failing field to an array of human-readable messages:
...
```json
{
    ...
    "errors": {
        ...
        "status": ["The selected status is invalid."],
        "customer_id": ["The customer id field must exist in customers."]
    }
}
```
...
- Messages are localized — Dhivehi by default, English when `?lang=en`
- Each field maps to an **array** of messages (there may be more than one rule failure per field)
- Custom messages can be added to any model via `validationMessages()`:
...
public static function validationMessages(): array
{
    return [
        'invoice_number.required' => [
            ...
            'dv' => 'އިންވޮއިސް ނަންބަރު ލިޔުން ލާޒިމް.',
        ],
    ];
}
```

- Server-side rules are defined in `baseRules()`, `rulesForCreate()`, or `rulesForUpdate()` — **not** in the `validationRule` key of `apiSchema()` (that key is frontend-only metadata)

---

### Common apiSchema Mistakes

The table below maps schema authoring mistakes to the runtime error or silent misbehaviour they cause.

<details open>
<summary><strong>Mistake → consequence table</strong></summary>

| Mistake | What breaks | Error / symptom | Fix |
|---------|-------------|-----------------|-----|
| `displayType: 'chip'` without a `chip` or `displayProps` key | Table display | ViewConfigValidator warning; frontend renders plain text | Add a `chip` map covering all possible values |
| `displayType: 'select'` without a `select` or `displayProps` key | Table display | ViewConfigValidator warning; display falls back to plain text | Add the matching `select` or `displayProps` config |
| `inputType: 'select'` without a `select` or `filterable` key | Form + filter | ViewConfigValidator error; form field may render without options | Add a `select` block with the correct `mode` |
| `select.mode: 'url'` without a `url` key | CCS response | `InvalidArgumentException` → CCS returns `422 {"error": "..."}` | Add `'url' => '/path/to/endpoint'` to the `select` block |
| `select.mode: 'relation'` without `relationship` key | Form dropdown | ViewConfigValidator warning; CCS guesses model name from column (e.g. `country_id` → `Country`) — may work but is fragile | Add `'relationship' => 'methodName'` matching the Eloquent method |
| `select.mode: 'self'` with empty or missing `items` | Form dropdown | ViewConfigValidator error; dropdown renders empty | Add at least one item to `items` |
| Model does not extend `BaseModel` | All GAPI endpoints | Query scopes (`selectColumns`, `search`, `sort`, `filter`, etc.) do not exist — runtime errors on any GAPI call | Extend `Ogp\UiApi\Models\BaseModel` |
| Model extends `BaseModel` but has no `apiSchema()` | CCS endpoint | `422 {"error": "Model 'Foo' not found or missing apiSchema()"}` | Implement `public function apiSchema(): array` |
| Column missing from `apiSchema()` but listed in view config `columns` | CCS endpoint | `422 {"error": "Column 'foo' is not defined in Foo apiSchema"}` | Add the column definition to `apiSchema()` |
| `formField` not set (defaults to `false`) | Forms | Column silently absent from auto-generated forms | Set `'formField' => true` on columns that should be editable |
| Missing `lang` array | All language-filtered views | Column appears in all language contexts — may cause LTR/RTL content mixing | Add `'lang' => ['en', 'dv']` (or the appropriate subset) |
| `key` does not match the array key | Headers, forms, filters | Header `value` / field `key` mismatches the actual data key — frontend bindings break | Set `'key'` to match the column map key exactly |
| Chip map missing a possible column value | Table chip display | Chip component receives an unknown value and renders nothing or a fallback | Add an entry for every value the column can hold |
| `validationRule` used for server-side enforcement | Server validation | Rules in `validationRule` are **not** applied by the server — only metadata for the frontend | Define server-side rules in `baseRules()` / `rulesForCreate()` / `rulesForUpdate()` |
| `rulesForCreate()` has `unique` rule without `rulesForUpdate()` ignoring it | Update endpoint | Updating an existing record triggers a unique violation on its own value | Use `Rule::unique('table')->ignore($id)` in `rulesForUpdate()` |
...
Two configuration flags help diagnose errors during development:

#### `APP_DEBUG=true` (`.env`)

When enabled, the raw PHP exception message is appended to the `message` field in GAPI error responses:

```json
{
    "message": "Unable to save the record. SQLSTATE[23000]: Integrity constraint violation: 1062 Duplicate entry 'INV-001' for key 'invoices_invoice_number_unique'"
}
```

This is safe for local development but **must be disabled in production** to avoid leaking schema information.

#### `debug_level` (`config/uiapi.php`)

| Value | Behaviour |
|-------|-----------|
| `1` (default) | No validation on CCS requests |
| `2` | CCS runs `ViewConfigValidator` on every request; returns `422` if errors found; attaches `_validation_warnings` to successful responses |

Set `debug_level` to `2` while building view configs to catch mistakes immediately:

```php
// config/uiapi.php
'debug_level' => env('UIAPI_DEBUG_LEVEL', 1),
```
