...
Complete guide to defining `apiSchema()` on your Eloquent models for use with the UiApi package. This document covers every key, its purpose, accepted values, defaults, and what happens when it is missing.
...
- [Column Definition Keys — Full Reference](#column-definition-keys--full-reference)
  - [hidden](#hidden)
  - [key](#key)
  - [label](#label)
  - [type](#type)
...
- [Display Configuration](#display-configuration)
  ...
  - [chip — Chip Display](#chip--chip-display)
  - [displayProps — Generic Display Properties](#displayprops--generic-display-properties)
  - [How displayType Resolution Works](#how-displaytype-resolution-works)
- [Select & Dropdown Configuration](#select--dropdown-configuration)
  - [The select Key](#the-select-key)
  - [Mode: self](#mode-self)
  - [Mode: relation](#mode-relation)
  - [Mode: url](#mode-url)
  - [Common Select Sub-Keys](#common-select-sub-keys)
- [Filterable Configuration](#filterable-configuration)
  - [The filterable Key](#the-filterable-key)
  - [filterable vs select — When to Use Which](#filterable-vs-select--when-to-use-which)
  - [filterable.type Values](#filterabletype-values)
- [Language Support](#language-support)
  - [How lang Filtering Works](#how-lang-filtering-works)
  - [Columns Without lang](#columns-without-lang)
  - [Language-Specific Columns](#language-specific-columns)
...
- Laravel Eloquent models, relationships, and migrations
- The UiApi package structure (see [UIAPI-README.md](../UIAPI-README.md))
- JSON view configs and the Component Config Service (CCS)
...
**How CCS processes chips:** When building headers, the CCS calls `normalizeDisplayConfig('chip', ...)` which collapses the localized `label` within each chip option to a single string for the requested language. So the frontend receives:
```json
{
  "chip": {
...
The `filterable` key is an alternative (and older) way to define how a column behaves in the **filter panel**. It shares many sub-keys with `select` but can additionally define a `type` that controls the filter input style.

**Structure:**
```php
'column_name' => [
    'filterable' => [
        'type' => 'search',          // Filter input type
        'label' => ['en' => '...'],  // Filter label override
        'value' => 'column_name',    // Which column to filter on
        // If type is 'select', same sub-keys as the select block:
        'mode' => 'self',
        'items' => [ ... ],
        'itemTitle' => ...,
        'itemValue' => '...',
...
**Example — select filter with relation mode:**
```php
'customer_id' => [
    'filterable' => [
        'mode' => 'relation',
...
**Example — select filter with self mode (full localized):**
```php
'gender' => [
    'filterable' => [
        'type' => 'select',
        ...
        'itemTitle' => ['dv' => 'itemTitleDv', 'en' => 'itemTitleEn'],
        'itemValue' => 'itemValue',
    ],
],
```
...
### filterable vs select — When to Use Which

Both `select` and `filterable` can define dropdown options. The CCS prioritizes them as follows:

```
Form fields:   select → filterable (fallback)
Filters:       inputType-based lookup → filterable (fallback)
```
...
|----------|----------------|
| Column needs a dropdown in **both** forms and filters with the **same** options | Use `select` with `inputType: 'select'` |
| Column needs **different** behavior in forms vs filters | Use `select` for form config, `filterable` for filter config |
| Column needs a filter but is **not** a form field | Use `filterable` alone |
| Legacy code compatibility | `filterable` works everywhere as a fallback |

**Key difference:** `filterable` can define a `type` key (`'search'`, `'select'`) that directly controls the filter input style. The `select` key does not have this — when using `select`, the filter type token is derived from `inputType`.

**Both can coexist** on the same column — `select` will be used for forms, and `filterable` provides explicit filter configuration.

---

### filterable.type Values

The `type` key inside `filterable` controls what kind of filter input is rendered:

| Value | Filter renders as |
|-------|-------------------|
| `'search'` | Text search input |
| `'select'` | Dropdown select (needs `mode`, `items`, etc.) |

When `filterable.type` is not set, the CCS infers the filter type from `inputType`:

| `inputType` | Filter type token |
|:---:|:---:|
| `'select'` | `Select` |
| `'textField'` | `Text` |
| `'numberField'` | `Number` |
| `'checkbox'` | `Checkbox` |
| `'datepicker'` | `Date` |
| default | Inferred from `type` (`date`/`datetime` → `Date`, else `Text`) |
...
| Key | Type | Required | Description |
|-----|------|----------|-------------|
| `type` | `string` | No | `'search'` or `'select'`. Determines the filter input type. |
| `label` | `string` or `array` | No | Override label for the filter. |
| `value` | `string` | No | Which column to filter on (defaults to the column `key`). |
| `mode` | `string` | For select type | `'self'`, `'relation'`, or `'url'`. Same as select modes. |
| `items` | `array` | For self mode | Static options array. Same format as select items. |
| `itemTitle` | `string` or `array` | Recommended | Display text field. Same behavior as in select. |
| `itemValue` | `string` | Recommended | Value field. Same behavior as in select. |
| `relationship` | `string` | For relation mode | Eloquent relationship method name. |
| `url` | `string` | For url mode | Custom URL to fetch filter options. |
| `multiple` | `bool` | No | Allow multiple filter selections. |
| `sourceModel` | `string` | No | Hint for the related model name. |
