...
**The key idea:** You never write the full UI payload by hand. You declare *what* to show (columns, filters, overrides) and CCS builds the *how* (headers array, filter objects, data links, form fields) from the model's schema and built-in component templates.
...
CCS normalizes the model name by stripping hyphens, underscores, and spaces, then lowercasing. So `criminal-record`, `CriminalRecord`, and `criminalrecord` all resolve to `criminalrecord.json`.
...
When using the `"ModelName/componentKey"` format, CCS checks if the model name matches the current model. If it does, it's a self-reference and looks up the root-level `componentKey` in the same file. If it's a different model name, CCS loads that model's view config file.

**Recommendation:** Use the `"ModelName/componentKey"` format for clarity, even for self-references. It makes it explicit which model the component belongs to.
