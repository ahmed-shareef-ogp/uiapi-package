...
<p>
    <img alt="Manual Version v0.7" src="https://img.shields.io/badge/Manual-v0.7-F59E0B?style=for-the-badge">
</p>
...
Additionally, `button` is a supported child `stackType` inside `row.fields` (both top-level rows and rows nested under a `section`). See the [`button` subsection](#button-inside-rowfields).
...
```json
{
    ...
    "fields": [
        ...
        {
            "stackType": "button",
            "label": { "en": "Details", "dv": "ތަފްޞީލު" },
            "color": "success",
            "size": "xs",
            "target": "_blank",
            "linkKey": "/page/CForm/:id/show"
        }
    ]
}
```
...
- a button object with `stackType: "button"` — see the next subsection

##### `button` (inside `row.fields`)

Buttons are not a top-level stack type. They appear as children of a `row` (either directly under `cardLayout.stacks` or nested inside `section.stacks[].fields`). A single button supports **two modes** selected with the `action` key:

| `action` | Behavior |
|---|---|
| *(omitted)* or `"redirect"` | Resolves `linkKey` and bubbles an `action-click` event up to `ListView`. Absolute URLs (`http…`) or `target: "_blank"` open in a new window; internal paths are pushed to the router. **This is the default and remains backwards compatible.** |
| `"viewPdf"` | Resolves `linkKey` to a PDF URL and opens it inline in the same `DocumentViewer` modal used by the table's `docButton` column. Does not emit `action-click`. |
...
| Key | Type | Required | Purpose |
|---|---|---|---|
| `stackType` | `string` | Yes | Must be `"button"` |
| `action` | `string` | No | `"redirect"` (default) or `"viewPdf"` |
| `label` | `{en, dv}` or `string` | Yes | Button label; localized object picks a value per language |
| `linkKey` | `string` | Yes | Either a dot-notated data path on the item (e.g. `"cv.attachment.url"`) or a URL template with `:param` / `{param}` placeholders (e.g. `"/page/CForm/:id/show"`). `:param` placeholders are substituted from the current item's top-level fields. |
| `color` | `string` | No | Button color token, e.g. `"primary"`, `"info"`, `"success"` |
| `size` | `string` | No | Button size token, e.g. `"xs"`, `"sm"` |
| `target` | `string` | No | *(redirect only)* — forces `window.open` when set to `"_blank"` |
| `titleKey` | `string` | No | *(viewPdf only)* — dot-notated data path whose value becomes the modal title. Falls back to the localized `label` when omitted. |
| `showDownload` | `boolean` | No | *(viewPdf only)* — show the download button in the viewer toolbar. Default `true`. |
| `showPrint` | `boolean` | No | *(viewPdf only)* — show the print button in the viewer toolbar. Default `true`. |

**`linkKey` resolution rules (applies to both modes):**
1. If the value starts with `/` or `http`, it is treated as a **URL template** — `:param` and `{param}` placeholders are replaced with the corresponding top-level properties of the item (e.g. `item.id`, `item.uuid`).
2. Otherwise it is treated as a **dot-notated data path** and the URL is read from the item (supports nested paths like `cv.attachment.url`).

This mirrors how the `table.columnCustomizations.<col>.displayType = "docButton"` opens PDFs in `DataTable.vue` — the card version uses the same underlying `DocumentViewer` component, so the viewing experience is identical across table and card views.
...
- A nested `row` inside a section can contain `button` children in its `fields` — both `redirect` and `viewPdf` actions work exactly as they do at the top level, resolved against the nested section item rather than the parent record.

Supported nested section `stackType` values:
- `field`
- `row` (may contain `button` children in its `fields`)
...
- A row field with `stackType: "button"` can be used for card actions. Set `action: "viewPdf"` to open a PDF in an inline `DocumentViewer` modal, or omit `action` (or set it to `"redirect"`) to navigate — internal paths go through the router, external URLs or `target: "_blank"` open in a new window.
- For `viewPdf` buttons, `linkKey` may be either a dot-path to a PDF URL on the item (e.g. `"cv.attachment.url"`) or an absolute URL template. The optional `titleKey` supplies the modal title from the item; when omitted, the button's localized `label` is used instead.
